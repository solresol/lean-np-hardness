...
| NP transport along reductions | Pending | Preprocessing, ordered-input transfer, reduction execution, and standalone reduced-output/certificate reassembly are checked. Lift the existing dispatcher to the output-extended stack family, redirect reduction halt into reassembly, prove the complete polynomial runtime, and combine the adapter with the checked certificate-bound transport. |
